...
const BaseSpinner = memo<{
  ...
  <div className="flex flex-col items-center justify-center space-y-3">
    ...
    {text && (
      <p
        className={`text-sm ${COLOR_CLASSES[color]} animate-pulse`}
        role="status"
      >
        {text}
      </p>
    ...
    if (fullScreen) {
      return (
        <div
          ...
          aria-live="polite"
        >
          <BaseSpinner text={text} color={color} size={size} />
        </div>
      );
    }
 ...
 */
export const PageLoading: React.FC<{ text?: string; className?: string }> =
  memo(({ text = '页面加载中...', className = '' }) => (
    <div
 ...
 */
export const CardLoading: React.FC<{ className?: string }> = memo(
  ({ className = '' }) => (
...
export const TableLoading: React.FC<{
  ...
  className?: string;
  showHeader?: boolean;
}> = memo(({ rows = 5, columns = 4, className = '', showHeader = true }) => (
